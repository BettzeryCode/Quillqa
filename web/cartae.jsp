...
setTimeout ("redireccionar()", 500); //tiempo expresado en milisegundos
  ...
  cn222=DriverManager.getConnection("jdbc:mysql://localhost/quillqa","root","root");
  cmd222=cn222.createStatement();
  String dni1="";
  String nombre1= "";
  rs222=cmd222.executeQuery("select dni, nombre from log ");  
                                    while(rs222.next()){ 
                                        dni1 = rs222.getString(1);
                                        nombre1 = rs222.getString(2);
                                      } 
...
<button  href="#modalSolicitud"  id = "bntn" class="btn btn-primary" data-toggle="modal" type="submit" style="display:none;" >Pedidocurso</button>&nbsp;
  ...
  rs5=cmd5.executeQuery("select PRODUCTO, SUBCAL, CANTIDAD, SUBTOTAL, TIEMPO from PRODUCTO_PERSONALIZADO");  
                            ...
                            <form name="form19" method ="post" id="registrar-form" action="cartaf.jsp">       
                                <center><button type="submit" id="terminarPedido" class="btn btn-danger" >Terminar</button></center>
                            </form>
                            </div>
                        </div>
                </div>
